...
SERVICE_MAP = [{"title": "Altenmarkt an der Triesting",
...
class CitiesApps:
    def __init__(self, password, email=None, phone=None) -> None:
        ...
        if r.status_code == 400:
            raise Exception(
                "failed to login to the App, check your login credentials")
        r.raise_for_status()

        self._session.headers.update(
            {"authorization": r.headers["access-token"]})
    ...
    def get_garbage_calendars(self, city_id: str) -> list:
        ...
        r = self._session.get(
            "https://api.citiesapps.com/garbagecalendars/filter",
            params=params_str)
        r.raise_for_status()
    ...
    def get_streets(self, city_id):
        ...
        r = self._session.get(
            "https://api.citiesapps.com/garbageareas",
            params=params)
        r.raise_for_status()
        return r.json()
    ...
    def generate_service_map(self) -> list[dict[str, str]]:
        ...
        for id, city in enumerate(supported_dict["supported"]):
            ...
            domain_end = slash_index[2] if len(
                slash_index) > 2 else len(city_homepage)
            url = city_homepage[:domain_end]
            # city["country_abbreviation"] returns "de" instead of "at"
            # sometimes
            service_map.append(
                {"title": city["name"], "url": url, "country": "at"})
        return service_map
