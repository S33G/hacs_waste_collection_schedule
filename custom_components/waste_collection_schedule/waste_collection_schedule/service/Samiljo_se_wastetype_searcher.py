...
parser.add_argument(
    "--street",
    type=str,
    help="Specify a street adress or part of it.")
parser.add_argument("--city", type=str, help="Specify a city.")
parser.add_argument(
    "--char",
    type=str,
    help="exclude characters from search (eg \"abcdef\")")
...
with alive_bar(bar_count) as bar:
    for char in alphabet:
        if not used_char:
            # if no used characters add a addresslist to checked list without
            # check aggainst checked_list
            ...
            for line in new_addressarray:
                # if line contains an alredy searched character then don"t
                # write it
                found_char = 0
                for x in used_char:
                    if x in line.strip("\n"):
