...
class Source:
    ...
    def fetch(self):
        ...
        if not self._uprn:
            ...
            if self._name:
                self._uprn = [
                    x["Uprn"] for x in addresses if (
                        x["Title"]).upper().startswith(
                        self._name.upper())][0]
            elif self._number:
                self._uprn = [x["Uprn"] for x in addresses if (
                    x["Title"]).startswith(str(self._number))][0]
        ...
        for collection in collections:
            ...
            if collection[0].__contains__(' and '):
                collections.append([collection[0].split(" and ", 2)[0].title().replace(
                    " Waste", ""), collection[1], collection[2], collection[3]])
                collections.append([collection[0].split(" and ", 2)[1].title().replace(
                    " Waste", ""), collection[1], collection[2], collection[3]])
            else:
                if collection[3] != "":
                    nextDate = datetime.datetime.strptime(
                        collection[3], "%d/%m/%Y").date()
                elif collection[1] == "WEEKLY":
                    d = datetime.date.today()
                    nextDate = d + \
                        datetime.timedelta((DAYS.index(collection[2].upper()) + 1 - d.isoweekday()) % 7)
