...
class Source:
    ...
    def fetch(self):
        ...
        soup = BeautifulSoup(
            json.loads(
                r.text)["ajax/houseNumbers"],
            features="html.parser")

        if soup.find("div", attrs={"class": "alert"}) is not None or soup.find(
                "option", string="Kérem, előbb válasszon közterületet") is not None:
            raise Exception(
        ...
        soup = BeautifulSoup(
            json.loads(
                r.text)["ajax/calSearchResults"],
            features="html.parser")

        if soup.find("div", attrs={"class": "alert"}) is not None:
            raise Exception(
                "House number not found, available house numbers: " +
                available_numbers)
        ...
        communal = [
            i for i in filter(
                lambda div: div.text == "Kommunális",
                communal_divs)]
        green = [
            i for i in filter(
                lambda div: div.text == "Zöld",
                communal_divs)]
            ...
            entries.append(
                Collection(
                    date=datetime.datetime.strptime(
                        element.parent.parent.findAll("td")[1].text,
                        "%Y.%m.%d").date(),
                    t="Communal",
