...
class Source:
    ...
    def fetch(self):
        ...
        # Load the Map Project (further requests don't appear to work if this
        # request is not made)
        ...
        entries.append(Collection(datetime.strptime(general_recycling_dates_text.split(
            " ")[4][:-1], "%d-%b-%Y").date(), "Recycling"))
        entries.append(Collection(datetime.strptime(
            general_recycling_dates_text.split(" ")[-1], "%d-%b-%Y").date(), "Recycling"))

        # glass recycling (blue lid)
        glass_recycling_dates_text = response['infoPanels']['info1']['feature']['fields'][4]['value']['value']
        entries.append(Collection(datetime.strptime(
            glass_recycling_dates_text.split(" ")[4][:-1], "%d-%b-%Y").date(), "Glass"))
        entries.append(Collection(datetime.strptime(
            glass_recycling_dates_text.split(" ")[-1], "%d-%b-%Y").date(), "Glass"))
