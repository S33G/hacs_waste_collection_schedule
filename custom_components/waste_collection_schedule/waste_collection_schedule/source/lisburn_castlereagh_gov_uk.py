    ...
    def fetch(self):
        ...
        if not self._property_id:
            ...
            for li in soup.findAll("li"):
                link = li.findAll("a")[0]
                property_id = link.attrs["href"].replace(
                    "/view", "").replace("/", "")
                address = link.text
            ...
            common = difflib.SequenceMatcher(
                a=all_addresses[0], b=all_addresses[1]).find_longest_match()
            to_be_removed = all_addresses[0][common.a:common.a + common.size]

            ids_by_house_number = {
                address.replace(
                    to_be_removed,
                    ""): property_id for property_id,
                address in address_by_id.items()}

            self._property_id = ids_by_house_number.get(
                str(self._house_number))

            if not self._property_id:
                raise ValueError(
                    f"Property not found for house number {self._house_number}")
