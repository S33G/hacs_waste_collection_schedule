...
def EXTRA_INFO():
    return [{"title": s["title"], "url": get_main_url(
        s["api_url"])} for s in SERVICE_MAP]
...
class Source:
    ...
    def fetch(self):
        ...
        # Something must be wrong, maybe the address isn't valid? No need to do
        # the extra requests so just return here.
        if len(data) == 0:
        ...
        # Retrieve the details about different waste management flows (for
