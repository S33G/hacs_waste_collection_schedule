...
TEST_CASES = {
    ...
    # If your property is listed here and you don't want it, please raise an
    ...
    def fetch(self):
        if self._uprn:
            ...
            req = urllib.request.Request(
                f"{API_URL}/property/{self._uprn}/calendar",
                headers=HEADERS)
            with urllib.request.urlopen(req) as response:
                html_doc = response.read()

            # Parse the page to get the data required (collection date and
            # type)
            ...
            for child in soup.find_all('div',
                ...
                try:
                    # There can be multiple bin collections on each day find
                    # them all
                    for collection in child.find_all('li'):
                        try:
                            # The collection details are in the title field of each ul/li
                            # Converting date from title field to a usable
                            # format
                            collection_date = parser.parse(
                                collection['title'].split(" - ")[0]).date()
                            # Getting the collection type
                            collection_type = collection['title'].split(
                                " - ")[1]
