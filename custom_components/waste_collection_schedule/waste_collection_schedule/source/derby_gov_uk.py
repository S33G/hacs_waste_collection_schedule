...
TEST_CASES = {
    # Derby City council wants specific addresses, hopefully these are generic
    # enough.
...
class Source:
    def __init__(
            self,
            premises_id: int = None,
            post_code: str = None,
            house_number: str = None):
        self._premises_id = premises_id
        self._post_code = post_code
        self._house_number = house_number
        if not any([self._premises_id,
                    self._post_code and self._house_number]):
            raise Exception(
    ...
    def fetch(self):
        ...
        for result in results:
            ...
            try:
                ...
                _LOGGER.info(
                    f"Skipped {date} as it does not match time format")
                continue
