...
TEST_CASES = {
    ...
    # Wednesday
    "9-19 McEwen": {"address": "9-19 MCEWEN DRIVE SUNBURY VIC 3429"},
...
if not isinstance(predict,         if )            raise Exception("'predict' must be a boolean value")
    ...
    def fetch(self):
        ...
        if self._predict:
            rub_dates = self.collect_dates(
    datetime.strptime(
        date_rubbish, "%d/%m/%Y").date(), 1)
            rec_dates = self.collect_dates(
    datetime.strptime(
        date_recycling, "%d/%m/%Y").date(), 2)
            grn_dates = self.collect_dates(
    datetime.strptime(
        date_green_waste, "%d/%m/%Y").date(), 2)
        else:
