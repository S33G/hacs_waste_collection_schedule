...
class Source:
    ...
    def __get_entries_for_street(
            self, year: int, today: datetime) -> list[dict[str, str | int]] | None:
        try:
            # get json file
        ...
        for ortsteil in data["allRegionen"][0]["regionen"][0]["ortsteile"]:
            if ortsteil["name"].lower().strip(
            ) != self._ortsteil.lower().strip():
                continue
            for strasse in ortsteil["strassen"]:
                if strasse["name"].lower().strip(
                ) != self._strasse.lower().strip():
                    continue
                return strasse["eintraege"]

    # generate dates, typematch and date_is_collection* are nearly 1:1
    # implementation of teir original crappy js code
    ...
    def __date_is_collection(self, node, day: datetime, exceptions) -> bool:
        return self.__date_is_collection_1_to_4(
            node, day, exceptions) or self.__date_is_collection5_6(
            node, day)
    ...
    def __date_is_collection_1_to_4(
        ...
        return (
            (
                (node["rhythmus"] == 1) and self.__typematch(
                    day.strftime("%m"),
                    day.strftime("%d"),
                    node["typ"],
                    2,
                ...
                ((node["tag1"] == dow) and (
                    node["tag2"] == 0)) or (
                        ((node["tag1"] == dow) and not weekeven) or (
                            (node["tag2"] == dow) and weekeven)))) or (
                                ((node["rhythmus"] == 2) and self.__typematch(
                                    day.strftime("%m"),
                                    day.strftime("%d"),
                                    node["typ"],
                                    3,
                                    ...
                                    self._rhythms[2],
                                    self._rhythms[4])) and (
                                        ((node["woche"] == 2) and not weekeven) or (
                                            (node["woche"] == 3) and weekeven)) and (
                                                node["tag1"] == dow)) or (
                                                    ((node["rhythmus"] == 3) and self.__typematch(
                                                        day.strftime("%m"),
                                                        day.strftime("%d"),
                                                        node["typ"],
                                                        4,
                                                        ...
                                                        self._rhythms[4])) and (
                                                            ...
                                                            4 == 0) and (
                                                                node["tag1"] == dow)) or (
                                                                    ((node["rhythmus"] == 4) and self.__typematch(
                                                                        day.strftime("%m"),
                                                                        day.strftime("%d"),
                                                                        node["typ"],
                                                                        1,
    ...
    def fetch(self):
        ...
        for year in years:
            ...
            for entry in entries:
                exceptions = {
                    datetime.strptime(
                        d_from, "%d.%m.%Y"): datetime.strptime(
                        d_to, "%d.%m.%Y") for d_from, d_to in json.loads(
                        entry["ausnahmen"]).items()}
                ...
                for day in self.__generate_dates(
                        today, datetime(year + 1, 1, 1)):
