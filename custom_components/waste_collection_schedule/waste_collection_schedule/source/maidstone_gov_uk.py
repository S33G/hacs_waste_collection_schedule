...
# Many thanks to dt215git for their work on the Bexley version of this
# provider which helped me write this.
...
TEST_CASES = {
    # has mutliple collections on same week per bin type
    ...
    def fetch(self):
        ...
        payload = {
            "formValues": {
                "Your collections": {
                    "address": {
                        "value": self._uprn}, "uprn": {
                        "value": self._uprn}}}}

        entries = []

        # Extract bin types and next collection dates, for some reason unlike
        # all others that use this service, you need to submit a bin type to
        # get useful dates.
        for bin in BIN_MAP.keys():
            ...
            for item in rowdata:
                ...
                # need to dedupe as MBC seem to list the same collection twice
                # for some places
